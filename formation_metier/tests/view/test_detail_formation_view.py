...
class DetailFormationViewTest(TestCase):
    ...
    def test_should_display_any_seance(self):
        ...
        self.assertContains(response, self.formation.code)
    ...
    def test_with_one_seance_for_formation(self):
        ...
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, seance.formation.name)
        self.assertContains(response, seance.formation.code)
        self.assertContains(response, seance.formation.description)
        self.assertNotContains(response, self.formation.name)
        self.assertContains(response, seance.participant_max_number)
        self.assertContains(response, seance.local)
        self.assertContains(response, seance.formateur)
        self.assertContains(response, seance.duree)
    ...
    def test_with_many_seance_not_all_for_formation(self):
        ...
        self.assertContains(response, self.formation.name)
        self.assertContains(response, self.formation.code)
        self.assertContains(response, seance2.local)
        self.assertContains(response, seance1.local)
        self.assertNotContains(response, seance3.local)
        self.assertNotContains(response, seance4.local)
